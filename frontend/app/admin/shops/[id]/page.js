...
export default function ShopReportPage() {
    ...
    const barberPerformance = entries.reduce((acc, entry) => {
        ...
        acc[barberName].revenue += entry.totalAmount;
        acc[barberName].entries += 1;
        return acc;
    }, {});
        ...
        <AdminLayout currentPage="shops">
            <div className="max-w-7xl mx-auto py-8 px-4">
                <div className="flex flex-col gap-6">
                    ...
                    {/* Barber Performance */}
                    {Object.keys(barberPerformance).length > 0 && (
                        <div className="p-6 bg-white rounded-xl border border-secondary-500">
                            <h2 className="text-lg font-semibold mb-4 text-primary-900">Barber Performance</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {Object.entries(barberPerformance).map(([barberName, stats]) => (
                                    <div key={barberName} className="p-4 bg-gray-50 rounded-lg border border-primary-900">
                                        <p className="text-primary-900 font-bold mb-2">{barberName}</p>
                                        <div className="flex justify-between">
                                            <div>
                                                <p className="text-secondary-500 text-xs">Entries</p>
                                                <p className="text-primary-900 font-bold">{stats.entries}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-secondary-500 text-xs">Revenue</p>
                                                <p className="text-primary-900 font-bold">£{stats.revenue.toFixed(2)}</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                                ...
                                <table className="w-full">
                                    <thead className="bg-secondary-500">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-sm font-semibold text-white">Barber</th>
